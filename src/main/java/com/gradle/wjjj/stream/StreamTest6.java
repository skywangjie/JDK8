...
public class StreamTest6 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(Stream.iterate(1, item -> item + 2).limit(6)
                .filter(item -> item > 2)//大于2的元素
                .mapToInt(i -> i * 2)//乘以2 这里不用map 避免自动装箱
                .skip(2)//跳过前2个元素
                .limit(2)
                .peek(e -> System.out.println("limit 2: " + e))//查看流经管道中某个点的元素
                .sum());
        //.reduce(0, Integer::sum));

        System.out.println(Stream.iterate(1, item -> item + 3).limit(6)
                .filter(item -> item > 2)//大于2的元素
                .mapToInt(i -> i * 2)//乘以2 这里不用map 避免自动装箱
                .skip(2)//跳过前2个元素
                .limit(2)
        ...
        //规避错误
        IntStream stream1 = Stream.iterate(1, item -> item + 2).limit(6)
                .filter(item -> item > 2)//大于2的元素
                .mapToInt(i -> i * 2)//乘以2 这里不用map 避免自动装箱
                .skip(2)//跳过前2个元素
                .limit(2);
